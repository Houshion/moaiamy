...
function redirect() {
    // if(isWX_Allipay() == 'WX'){
        location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx14c6b5d27daad538&redirect_uri=' + location.href + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
    // }else{
    //     dlctipbox.show('请在微信浏览器中打开');
    // }
}
//数据为空提示
...
function dlcUrl() {
    return 'http://moaianmoyi.app.xiaozhuschool.com/';
}
...
// 滚动条运动函数
function change(val_start, val_end, obj) {
    obj.width(val_start + '%') // 初始化
    obj.animate({ width: val_end + "%" }, 500)
}
//SHA1算法加密
(function(D) {
    function p(b, e, c) {
